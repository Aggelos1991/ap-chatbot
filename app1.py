# --------------------------------------------------------------
# ReconRaptor – FINAL MULTILINGUAL & LOGICALLY PERFECT
# --------------------------------------------------------------
...
st.markdown("<p style='text-align: center; font-size: 1.3rem; color: #555;'>Intelligent Vendor Invoice Reconciliation (ES/EN/GR)</p>", unsafe_allow_html=True)
...
def doc_type(row, tag):
    ...
    # PAYMENT KEYWORDS (Spanish, Greek, English)
    pay_keywords = [
        # Spanish
        ...
        # English
        "payment", "paid", "bank transfer", "wire", "deposit", "settlement", "receipt"
    ]
    ...
    is_payment_text = any(k in r for k in pay_keywords)
    is_net_credit = credit > debit and credit > 0.01
    has_invoice_ref = any(k in r for k in inv_keywords)

    if is_payment_text or (is_net_credit and not has_invoice_ref):
        return "PAYMENT"
...
def match_invoices(erp_df, ven_df):
    ...
    # CORRECT: Missing in ERP = Vendor has, ERP does NOT
    ...
    # CORRECT: Missing in Vendor = ERP has, Vendor does NOT
...
# ==================== PAYMENT MATCHING (EXCLUDES MATCHED INVOICES) ====================
def extract_payments(erp_pay_df, ven_pay_df, matched_erp_inv, matched_ven_inv):
    ...
    def references_matched_invoice(reason, inv_set):
        if pd.isna(reason): return False
        txt = str(reason).lower()
        for inv in inv_set:
            code = clean_invoice_code(inv)
            if code and code in txt.replace(" ", ""):
                return True
            if str(inv).lower() in txt:
                return True
        return False

    erp_pay_df = erp_pay_df[~erp_pay_df["reason_erp"].apply(lambda x: references_matched_invoice(x, matched_ven_inv))]
    ven_pay_df = ven_pay_df[~ven_pay_df["reason_ven"].apply(lambda x: references_matched_invoice(x, matched_erp_inv))]

    matched = []
    used = set()
    for _, e in erp_pay_df.iterrows():
        e_amt = round(e["Amount"], 2)
        for vi, v in ven_pay_df.iterrows():
            if vi in used: continue
            v_amt = round(v["Amount"], 2)
            if abs(e_amt - v_amt) < 0.05:
                matched.append({
                    "ERP Reason": str(e.get("reason_erp", ""))[:100],
                    "Vendor Reason": str(v.get("reason_ven", ""))[:100],
                    "ERP Amount": e_amt,
                    "Vendor Amount": v_amt,
...
if uploaded_erp and uploaded_vendor:
    try:
        ...
        st.markdown('<h2 class="section-title">Payment Transactions (Unreferenced)</h2>', unsafe_allow_html=True)
        if not pay_match.empty:
            st.dataframe(style(pay_match, "background:#004D40;color:#fff;font-weight:bold;"), use_container_width=True)
            st.success(f"{len(pay_match)} unmatched payments reconciled.")
        else:
            st.info("No unmatched payments (all reference reconciled invoices).")
