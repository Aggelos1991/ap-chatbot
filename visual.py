...
if uploaded_file:
    try:
        ...
        clicked = st.session_state.clicked_vendor
        if clicked:
            st.subheader(f"Raw Invoices – {clicked}")
            raw = df[df['Vendor_Name'] == clicked][['VAT_ID','Due_Date','Open_Amount','Status','Vendor_Email','Account_Email']]
    ...
    st.info("Upload Excel → Click bar → Vendor details → Export emails")
